...
use vctcareer_backend::simulation_manager::{
    ...
    get_live_stats, get_round_summary, get_scoreboard, get_simulation_events, get_simulation_state,
...
fn main() {
    ...
    println!(
        "✓ Current state: Round {}, Score: {}-{}, Ticks: {}",
    ...
    println!(
        "  - Attacker Credits: {} ({})",
        economy.attacker_average_credits, economy.attacker_buy_strength
    );
    println!(
        "  - Defender Credits: {} ({})",
        economy.defender_average_credits, economy.defender_buy_strength
    );
    println!(
    ...
    let checkpoint_id = create_checkpoint(
        &manager,
        &simulation_id,
        Some("Mid-match checkpoint".to_string()),
    )
    ...
    println!(
        "✓ Found {} events around timestamp {} (±5s window)",
        events_at_time.len(),
        current_time
    );
    ...
    let round1_events =
        get_events_by_round(&manager, &simulation_id, 1).expect("Failed to get round 1 events");
    println!("✓ Round 1 events: {}", round1_events.len());

    // Get events for specific high-skill players (TenZ and yay)
    let star_player_events =
        get_events_by_player(&manager, &simulation_id, 1).expect("Failed to get TenZ events");
    println!("✓ TenZ (Player 1) events: {}", star_player_events.len());

    let yay_events =
        get_events_by_player(&manager, &simulation_id, 6).expect("Failed to get yay events");
    println!("✓ yay (Player 6) events: {}", yay_events.len());

    // Get only kill events to analyze frags
    let kill_events =
        get_events_by_type(&manager, &simulation_id, "Kill").expect("Failed to get kill events");
    ...
    let round_summary =
        get_round_summary(&manager, &simulation_id, 1).expect("Failed to get round summary");
    ...
    println!(
        "✓ Kill/Damage events for TenZ and yay: {}",
        filtered_events.len()
    );
    ...
    let final_stats =
        get_simulation_stats(&manager, &simulation_id).expect("Failed to get final stats");

    println!("Player Performance Summary:");
    println!(
        "{:<15} {:<8} {:<8} {:<8} {:<10} {:<8} {:<8}",
        "Name", "Kills", "Deaths", "Assists", "Damage", "HS%", "Credits"
    );
    println!("{}", "-".repeat(75));
