...
fn main() {
    ...
    sim.add_player(Player::new(1, "Jett A".to_string(), Agent::Jett, Team::Attackers, 0.8, 0.7, 0.6, 0.75));
    sim.add_player(Player::new(2, "Phoenix A".to_string(), Agent::Phoenix, Team::Attackers, 0.65, 0.5, 0.7, 0.8));
    sim.add_player(Player::new(3, "Sova A".to_string(), Agent::Sova, Team::Attackers, 0.7, 0.6, 0.65, 0.9));
    sim.add_player(Player::new(4, "Sage A".to_string(), Agent::Sage, Team::Attackers, 0.5, 0.4, 0.5, 0.85));
    sim.add_player(Player::new(5, "Viper A".to_string(), Agent::Viper, Team::Attackers, 0.75, 0.65, 0.55, 0.8));

    // Defenders with varied skills
    sim.add_player(Player::new(6, "Omen D".to_string(), Agent::Omen, Team::Defenders, 0.7, 0.6, 0.75, 0.8));
    sim.add_player(Player::new(7, "Killjoy D".to_string(), Agent::Killjoy, Team::Defenders, 0.6, 0.55, 0.6, 0.9));
    sim.add_player(Player::new(8, "Cypher D".to_string(), Agent::Cypher, Team::Defenders, 0.65, 0.5, 0.7, 0.85));
    ...
    for event in sim.events {
        match event {
            ...
            VCTCareerBackend::sim::GameEvent::MatchEnd { timestamp, winning_team, score_attackers, score_defenders } => {
                println!("[{}] Match End - Winner: {:?} | Final Score: {}-{}", timestamp, winning_team, score_attackers, score_defenders);
            }
            VCTCareerBackend::sim::GameEvent::BuyPhaseStart { timestamp, round_number } => {
                println!("[{}] Round {} Buy Phase Start", timestamp, round_number);
            }
            VCTCareerBackend::sim::GameEvent::BuyPhaseEnd { timestamp, round_number } => {
                println!("[{}] Round {} Buy Phase End", timestamp, round_number);
            }
            VCTCareerBackend::sim::GameEvent::RoundStart { timestamp, round_number, attacker_credits_start, defender_credits_start } => {
                println!("[{}] Round {} Start | Attacker Credits: {} | Defender Credits: {}", timestamp, round_number, attacker_credits_start, defender_credits_start);
            }
            VCTCareerBackend::sim::GameEvent::RoundEnd { timestamp, round_number, winning_team, reason } => {
            ...
            VCTCareerBackend::sim::GameEvent::SideSwap { timestamp, round_number } => {
                println!("[{}] Side Swap at Round {} - Teams switching sides", timestamp, round_number);
            }
        }
    }
}