...
fn safe_lock<T>(mutex: &Arc<Mutex<T>>) -> Result<MutexGuard<T>, String> {
    mutex.lock().map_err(|e| format!("Failed to acquire lock: {}", e))
}
...
pub fn advance_simulation_legacy(
    ...
    let advance_mode = AdvanceMode::from_string(
        mode.as_deref().unwrap_or("tick"), 
        ticks
    );
...
pub fn get_round_summary(
    ...
    let simulation_id = Uuid::parse_str(simulation_id_str)
        .map_err(|_| "Invalid simulation ID format".to_string())?;
    let simulations = safe_lock(manager)?;
    let _sim = simulations.get(&simulation_id)
        .ok_or("Simulation not found")?;
...
pub fn get_live_stats(
    ...
    let mut performers: Vec<PlayerPerformance> = player_stats.iter().map(|stats| {
        ...
        let rounds_played = if state.current_round > 0 { state.current_round } else { 1 };
        let avg_damage = stats.damage_dealt as f32 / rounds_played as f32;
        
        // Get player name
        let player_name = sim.players.get(&stats.player_id)
            .map(|p| p.name.clone())
...
pub fn get_scoreboard(
    ...
    for event in &sim.events {
        match event {
            GameEvent::RoundStart { timestamp, round_number, .. } => {
                current_round = *round_number;
                round_start_time = *timestamp;
            }
            GameEvent::RoundEnd { timestamp, round_number, winning_team, reason } => {
    ...
    let mut rankings: Vec<PlayerRanking> = player_stats.iter().map(|stats| {
        // Calculate rating (simplified combat score)
        let rating = (stats.kills as f32 * 2.0) + (stats.assists as f32 * 0.5) + 
...
impl EventStream {
    ...
    pub fn unsubscribe(&mut self, listener_id: &str) {
        self.subscribers.retain(|sub| sub.listener_id != listener_id);
    }
