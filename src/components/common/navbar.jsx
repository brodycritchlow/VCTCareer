...
const Navbar = ({ navigation, profileNavigation, logoSrc, profileIconSrc, hideUserProfile }) => {
    const currentNavigation = navigation;
    const currentProfileNavigation = profileNavigation;

    const finalLogoSrc = logoSrc || "https://placehold.co/32x32/indigo/white?text=LOGO";
    const finalProfileIconSrc = profileIconSrc || "https://placehold.co/32x32/gray/white?text=USER";

    return (
        <Disclosure as="nav" className="bg-gray-800 font-sans sticky top-0 z-50">
            {/* Main Navbar Row: Logo (left), Navigation (centered), Profile (right) */}
            {/* Removed px- classes from here to allow content to truly pin to edges */}
            <div className="relative flex h-16 items-center justify-between">

                {/* Left Group */}
                <div className="flex items-center pl-2 sm:pl-6 lg:pl-8 z-10">
                    {/* Mobile menu button - positioned absolutely relative to its parent */}
                    <div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
                        <DisclosureButton className="group relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-inset">
                            ...
                            <Bars3 aria-hidden="true" className="block size-6 group-data-[open]:hidden" />
                ...
                <div className="absolute left-0 right-0 flex justify-center pointer-events-none">
                    <div className="flex space-x-4 pointer-events-auto">
                        {currentNavigation && currentNavigation.map((item) => (
                            <a
                                key={item.name}
                                href={item.href}
                ...
                {!hideUserProfile && (
                    <div className="flex items-center pr-2 sm:pr-6 lg:pr-8 z-10">
                        ...
                        <Menu as="div" className="relative ml-3">
                            ...
                            >
                                {currentProfileNavigation && currentProfileNavigation.map((item) => (
